...
// ── Mã hoá AES-256-CBC + gzip + base64 ───────────────────────────────────────
...
<soap:Envelope xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
  ...
  <soap:Body>
    ...
    curl_setopt_array($ch, [
        ...
        CURLOPT_CONNECTTIMEOUT => BKAV_TIMEOUT_CONNECT,
        CURLOPT_TIMEOUT        => BKAV_TIMEOUT_READ,
...
function numberToWordsVN(float $amount): string {
    ...
    foreach (array_reverse($groups, true) as $i => $g) {
        if ($g > 0) {
            $parts[] = $readThree($g) . $units[$i];
        }
    }

    $result = ucfirst(implode(' ', $parts)) . ' đồng';
    return $result;
}

// ── Xử lý response SOAP từ BKAV ─────────────────────────────────────────────
function parseBkavResponse(string $rawXml): array {
    ...
    // Parse JSON (trực tiếp hoặc decrypt)
...
function buildBkavPayload(array $inv): array {
    ...
    foreach ($inv['items'] as $idx => $it) {
        $items[] = [
            ...
            'TaxRateValue'  => (float) ($inv['vat_rate'] ?? 0),
            'TaxAmount'     => round((float)$it['total_price'] * ((float)($inv['vat_rate'] ?? 0) / 100)),
            'IsDiscount'    => false,
            'ItemOrder'     => $idx + 1,
        ];
    }
    ...
    return [
        ...
        'InvoiceTotalAmountInWords'      => $totalWords,
        'SellerTaxCode'                  => COMPANY_TAX,
        'SellerFullName'                 => COMPANY_NAME,
...
if ($method === 'POST') {
    ...
    try {
        ...
        bkav_log("Sending invoice_id={$invoiceId} invoice_no={$inv['invoice_no']}");

        $rawXml     = bkavSoapCall($encData);
        bkav_log("Raw response (first 500): " . substr($rawXml, 0, 500));

        $result     = parseBkavResponse($rawXml);
        bkav_log("Parsed result: " . json_encode($result));

        // Lấy số HĐ BKAV từ response
